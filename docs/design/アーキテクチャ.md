...
アセットのビルド方法は画面実装時に決める。ViteやTailwindのビルドにNode.jsが必要な場合はDocker側に構築する。静的チェック用にDockerの`node`サービス（Node.js 24）を用意しており、npmによるビルドにも利用できる。
...
認証方式や利用者の業務モデルは、該当する機能設計で決めてから必要な実装を追加する。
...
同期・非同期と再試行の方針は、待ち時間や信頼性の要件から決める。
...
静的チェックの必須手順と検査範囲は[コーディング規約](コーディング規約.md)に従う。
