...
class TopPageTest extends TestCase
{
    public function test_トップページに入力と出力の操作画面を表示する(): void
    {
        ...
        $response
            ...
            ->assertSee(route('character-directions.index'), false)
            ->assertSee(route('scene-directions.index'), false)
            ->assertSee(route('expressions.store'), false)
            ->assertSee(route('gazes.store'), false)
